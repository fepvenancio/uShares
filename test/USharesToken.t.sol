...
contract USharesTokenTest is BaseTest {
    ...
    function setUp() public override {
        ...
        bytes32 sourcePositionKey = positionManager.getPositionKey(tokenPool, SOURCE_CHAIN, SOURCE_CHAIN, address(vault));
    ...
    function testFuzz_CCTMessageId_Uniqueness(
        uint96 amount1,
        uint96 amount2,
        uint32 destChain1,
        uint32 destChain2
    ...
    function test_RevertCCTMint_ZeroAmount() public {
        ...
        token.releaseOrMint(ICCTToken.ReleaseOrMintParams({
    ...
    function test_RevertCCTBurn_ZeroAmount() public {
        vm.startPrank(tokenPool);
        vm.expectRevert(abi.encodeWithSelector(Errors.ZeroNumber.selector, 0));
        token.lockOrBurn(ICCTToken.LockOrBurnParams({
            sender: tokenPool,
            amount: 0,  // Zero amount
            destinationChainSelector: uint64(DEST_CHAIN),
            receiver: user2,
